...
################################ HSV ################################
    ...
    imagen_hsv = cv2.cvtColor(image, cv2.COLOR_RGB2HSV)
    ...
    mascara_fuego1 = cv2.bitwise_or(mascara1, mascara2)
    image = cv2.cvtColor(image, cv2.COLOR_RGB2BGR)  # Convertir de BGR a RGB
    resultado = cv2.bitwise_and(image, image, mask=mascara_fuego1)

    r, g, b = cv2.split(resultado)
    mask = b > 175
    # mask_final = mask & mascara_fuego
    mask_final = mascara_fuego1
    mask_final = mask_final.astype(np.uint8) * 255
    resultado[mask_final == 0] = 0

    fire_mask_f3 = mask_final & fire_mask
    fire_mask_f3 = fire_mask_f3.astype(np.uint8) * 255
    ################################ HSV ################################
...
output_folder = '/home/pi/TESIS/Codigos/Imagenes/Resultados/Color/Escenario1/H3'
os.makedirs(output_folder, exist_ok=True)
log_file = '/home/pi/TESIS/Codigos/Imagenes/Resultados/Color/Escenario1/H3/deteccion_fuego.txt'
...
def procesar_carpeta(carpeta, contiene_fuego_esperado, f):
    for filename in sorted(os.listdir(carpeta)):                           # Procesar imágenes en orden
        if filename.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            f.flush()
            time.sleep(0.045)  # Esperar 30 segundos antes de procesar la siguiente imagen
...
archivo_nuevo = '/home/pi/TESIS/Codigos/Metricas/Barras_y_Lineal/Metodo 13.csv'
archivo_original1 = '/home/pi/TESIS/Codigos/Metricas/Barras_y_Lineal/monitoreo.png'
archivo_nuevo1 = '/home/pi/TESIS/Codigos/Metricas/Barras_y_Lineal/Metodo 13.png'
