...
def detect_fire(image, R, G, B):                                                       # Detecta fuego en una imagen usando el modelo de color PJF
    ...
    ################################ HSI ################################
    H, S, I, image_hsi = rgb_to_hsi(image)                                      # Convertir imagen a escala de grises para binarización                                           # Umbrales basados en las ecuaciones del paper
    R_t = 125                                                                   # Umbral de R (ajustable según la iluminación)
    S_t = 0.2                                                                   # Umbral de saturación
    K = 0.15                                                                    # Umbras 0.15 - 2

    mask_1 = (R > G) & (G > B) & (R > R_t) & (S > 0.1) & (S > (255 - R) / 20) & (S >= (255 - R) * S_t / R_t)  # Aplicar criterio original RGB-HSI
    mask1_1 = (R > B) & (G > B) & (R >= R_t) & (S >= (255 - R) * np.mean(S) * K)
    mask2_1 = mask_1 & mask1_1
    flame_mask = np.zeros_like(R)                                               # Convertir la máscara binaria a imagen
    flame_mask[mask2_1] = 255
    flame_mask = flame_mask.astype(np.uint8) * 255

    fire_mask_f4 = flame_mask & fire_mask
    fire_mask_f4 = fire_mask_f4.astype(np.uint8) * 255
    ################################ HSI ################################
...
output_folder = '/home/pi/TESIS/Codigos/Imagenes/Resultados/Color/Escenario1/H4'
os.makedirs(output_folder, exist_ok=True)
log_file = '/home/pi/TESIS/Codigos/Imagenes/Resultados/Color/Escenario1/H4/deteccion_fuego.txt'
...
def procesar_carpeta(carpeta, contiene_fuego_esperado, f):
    for filename in sorted(os.listdir(carpeta)):                           # Procesar imágenes en orden
        if filename.lower().endswith((".jpg", ".png", ".jpeg")):
            ...
            f.flush()
            time.sleep(0.03)  # Esperar 30 segundos antes de procesar la siguiente imagen
...
archivo_nuevo = '/home/pi/TESIS/Codigos/Metricas/Barras_y_Lineal/Metodo 14.csv'
archivo_original1 = '/home/pi/TESIS/Codigos/Metricas/Barras_y_Lineal/monitoreo.png'
archivo_nuevo1 = '/home/pi/TESIS/Codigos/Metricas/Barras_y_Lineal/Metodo 14.png'
